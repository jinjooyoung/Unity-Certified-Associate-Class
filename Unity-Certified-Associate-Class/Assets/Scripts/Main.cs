...
public class Main : MonoBehaviour
{
    ...
    public string itemName = "포션";           // string (문자열) itemName (변수 이름) 선언하고 포션을 넣어줌

    public int HP = 100;                    // 체력 값 선언
    public float MoveDistance = 0.0f;       // 이동 값 선언

    public void Move()                      // 이동 함수 선언
    {
        HP = HP - 10;                       // 체력 10 감소
        MoveDistance = MoveDistance + 1;    // 이동 위치 1 증가
    }

    public void Move(int hp , int distance)         // 인수 입력 받는 함수 선언
    {
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.Alpha1))                // 1번 버튼을 눌렀을 때 (Input 클래스로 입력 감지)
        {
            Debug.Log(KeyCode.Alpha1.ToString() + " 버튼 눌림 ");
            Move();                                         // Move 함수 사용
        }

        if (Input.GetKeyDown(KeyCode.Alpha2))                // 2번 버튼을 눌렀을 때 (Input 클래스로 입력 감지)
        {
            Debug.Log(KeyCode.Alpha2.ToString() + " 버튼 눌림 ");
            Move(30, 1);                                         // Move 함수 사용
        }

        if (Input.GetKeyDown(KeyCode.Alpha3))                // 3번 버튼을 눌렀을 때 (Input 클래스로 입력 감지)
        {
            Debug.Log(KeyCode.Alpha3.ToString() + " 버튼 눌림 ");
            Move(15, 2);                                         // Move 함수 사용
        }

        if (Input.GetKeyDown(KeyCode.Alpha4))                // 4번 버튼을 눌렀을 때 (Input 클래스로 입력 감지)
        {
            int GetRandNumber = 0;                              // GetRandNumber (변수 선언)
            GetRandNumber = RandomNumber();                     // 변수에 int 함수 값을 Return 받아서 저장

            Debug.Log(" GetRandNumber : " + GetRandNumber);     // 해당 값을 콘솔 로그 창에 보여준다.
        }
    }
}
